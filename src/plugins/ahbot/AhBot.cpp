...
void AhBot::ForceUpdate()
{
    ...
    for (int i = 0; i < MAX_AUCTIONS; i++)
    {
        ...
        for (int j = 0; j < CategoryList::instance.size(); j++)
        {
            ...
            added += AddAuctions(i, category, &inAuctionItems, quality, sAhBotConfig.updateItemNum);
            ++quality;
        }
    }
...
int AhBot::AddAuctions(int auction, Category* category, ItemBag* inAuctionItems, uint8 quality, uint32 numbers)
{
    SQLTransaction Trans = CharacterDatabase.BeginTransaction();
    vector<uint32>& inAuction = inAuctionItems->Get(category);

    std::string name2 = category->GetName() + sAhBotConfig.GetType(quality);

    uint32 maxAllowedAuctionCount = sAhBotConfig.GetMaxAuctionNumber(name2.c_str(), 0); //    categoryMaxAuctionCount[category->GetName()];
    if (inAuctionItems->GetCount(category) >= int32(maxAllowedAuctionCount))
        return 0;

    int added = 0;
    vector<uint32> available = availableItems.Get(category);
    for (uint32 i = 0; i <= maxAllowedAuctionCount && available.size() > 0 && inAuctionItems->GetCount(category) < int32(maxAllowedAuctionCount) && i < numbers; ++i)
    {
        ...
        if (time(0) - sellTime > sAhBotConfig.itemSellMinInterval)
        {
        ...
        added += AddAuction(auction, category, proto, Trans);
    }
    if (added > 0)
...
int AhBot::AddAuction(int auction, Category* category, ItemTemplate const* proto, SQLTransaction& transItem)
{
    ...
    */
    ...
    uint32 price = proto->SellPrice;  // category->GetPricingStrategy()->GetSellPrice(proto, auctionIds[auction]);
    ...
    // price = category->GetPricingStrategy()->GetBuyPrice(proto, auctionIds[auction]); // == 0 ? price : category->GetPricingStrategy()->GetBuyPrice(proto, auctionIds[auction]);
    // if( price == 0 )
    ...
    auctionEntry->houseId = auctionIds[auction];
    ...
    item->SaveToDB(transItem);
    sAuctionMgr->AddAItem(item);
    auctionHouse->AddAuction(auctionEntry);
    auctionEntry->SaveToDB(transItem);

    auctionHouse->AddAuction(auctionEntry);
    // transItem->PAppend(trans);
    // CharacterDatabase.CommitTransaction(trans);
...
void AhBot::CheckCategoryMultipliers()
{
    ...
    CharacterDatabase.PExecute("TRUNCATE ahbot_category");
    // sAhBotConfig.ReloadConfigNumbers();
    uint8 quality = 1;
    for (int i = 0; i < CategoryList::instance.size(); i++)
    {
        if (quality > 4) quality = 1;
        string name = CategoryList::instance[i]->GetName();
        name += sAhBotConfig.QualityValue(quality);
        if (categoryMultiplierExpireTimes[name] <= time(0) || categoryMultipliers[name] == 0.0 || sAhBotConfig.GetMaxAuctionNumber(name.c_str(), 0))
        {
            categoryMultipliers[name] = (double)urand(20, 100) / 20.0; // Edited By XEQT
            uint32 maxAllowedAuctionCount = CategoryList::instance[i]->GetMaxAllowedAuctionCount();
            uint32 maxAllowedFromConfig = sAhBotConfig.GetMaxAuctionNumber(name.c_str(), 5);   //.GetMaxAllowedAuctionCount(name);
            categoryMaxAuctionCount[name] = maxAllowedFromConfig; //urand(maxAllowedAuctionCount, maxAllowedAuctionCount * 2); // urand(maxAllowedAuctionCount / 2, maxAllowedAuctionCount);
