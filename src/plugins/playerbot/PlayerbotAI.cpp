...
int32 PlayerbotAI::CalculateGlobalCooldown(uint32 spellid, SpellInfo* spellInfo)
{
    if (!spellid)
        return 0;

    if (spellInfo)
        return bot->GetSpellHistory()->GetRemainingCooldown(spellInfo);
...
bool PlayerbotAI::TellMaster(string text, PlayerbotSecurityLevel securityLevel)
{
    ...
    {
        if (!bot->isInFront(master, float(M_PI) / 2))
...
bool PlayerbotAI::CastSpell(uint32 spellId, Unit* target)
{
    ...
    if (!bot->isInFront(faceTo, float(M_PI) / 2))
    {
