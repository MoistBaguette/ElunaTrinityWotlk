...
void PlayerbotFactory::InitPet()
{
    ...
    if (!pet)
    {
        ...
        for (auto const& creatureTemplatePair : creatureTemplateContainer)
        {
            CreatureTemplate const& co = creatureTemplatePair.second;
            ...
            ids.push_back(creatureTemplatePair.first);
        }
    ...
    for(uint8 slot = 0; slot < EQUIPMENT_SLOT_END; ++slot)
    {
        ...
        do
        {
           ...
           for (auto const& itemTemplatePair : itemTemplates)
            {
                uint32 itemId = itemTemplatePair.first;
...
void PlayerbotFactory::InitTalents(uint32 specNo)
{
    ...
    for (map<uint32, vector<TalentEntry const*> >::iterator i = spells.begin(); i != spells.end(); ++i)
    {
        ...
        while (!spells.empty() && (int)freePoints - (int)bot->GetFreeTalentPoints() < 5 && attemptCount++ < 3 && bot->GetFreeTalentPoints())
        {
            ...
            int maxRank = 0;
            for (uint32 rank = 0; rank < min((uint32)MAX_TALENT_RANK, bot->GetFreeTalentPoints()); ++rank)
            {
...
void PlayerbotFactory::InitPotions()
{
    ...
    for (auto const& itemTemplatePair : itemTemplateContainer)
    {
