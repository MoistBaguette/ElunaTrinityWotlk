...
bool UseItemHearthStone::Execute(Event event)
{
    if (!ai->GetBot()->IsAlive() || ai->GetBot()->IsInCombat())
    {
        ai->TellMaster("You must be alive and not in combat to use Hearthstone");
        return false;
    }

    SpellEntry const* spellEntry = sSpellStore.LookupEntry(8690);
    SpellInfo* spellInfo = new SpellInfo(spellEntry);


    int32 cooldown = ai->CalculateGlobalCooldown(8690, spellInfo);
    if (cooldown > 0) // if (temp2->GetRecoveryTime() > 0)
    {
        ostringstream out; out << "There is cooldown left on Hearthstone" << "[" << (cooldown / 1000 / 60) << " min left]";
        ai->TellMaster(out.str());
        return false;
    }
    if (ai->CanCastSpell(spellInfo->Id, bot->ToUnit(), false))
    {
        bot->CastSpell(bot, spellInfo->Id);
        ai->TellMaster("Using Hearthstone.");
        return true;
    }
    ai->TellMaster("Something whent wrong HELP !!!");
    return false;
}
    ...
    *packet << bagIndex << slot << cast_count << uint32(0) << item_guid
    ...
    for (int i=0; i<MAX_ITEM_PROTO_SPELLS; i++)
    {
        ...
        if (pSpellInfo->Targets & TARGET_FLAG_ITEM)
        {
            ...
            Spell *spell = new Spell(bot, pSpellInfo, TRIGGERED_NONE, ObjectGuid::Empty);
